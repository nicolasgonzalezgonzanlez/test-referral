...
var ValidationService = /** @class */ (function () {
    ...
    ValidationService.prototype.ValidateParam = function (property, rawValue, name, fieldErrors, parent, minimalSwaggerConfig) {
        ...
        switch (property.dataType) {
            ...
            default:
                if (property.ref) {
                    return this.validateModel({ name: name, value: value, refName: property.ref, fieldErrors: fieldErrors, parent: parent, minimalSwaggerConfig: minimalSwaggerConfig });
                }
                return value;
        }
    };
    ...
    ValidationService.prototype.validateUnion = function (name, value, fieldErrors, swaggerConfig, subSchemas, parent) {
        ...
        subSchemas.forEach(function (subSchema) {
            ...
            if (Object.keys(subFieldError).length === 0) {
                cleanValues = __assign({}, cleanValues, cleanValue);
            }
        });
    ...
    ValidationService.prototype.validateIntersection = function (name, value, fieldErrors, swaggerConfig, subSchemas, parent) {
        ...
        var cleanValues = {};
        subSchemas
            .filter(function (subSchema) { return subSchema.ref; })
        ...
        var refNames = subSchemas.filter(function (subschema) { return subschema.ref; }).map(function (subschema) { return subschema.ref; });
        if (!refNames.every(function (refName) { return _this.models[refName]; })) {
            return value;
        }
        var reportedExcess = new Set(refNames
            .map(function (refName) { return _this.models[refName]; })
            .reduce(function (acc, subSchema) {
            return __spread(acc, _this.getExcessPropertiesFor(subSchema, Object.keys(value), swaggerConfig));
        }, []));
        if (reportedExcess.size === 0) {
            return value;
        }
        var allowedProperties = new Set(refNames
            .map(function (refName) { return _this.models[refName]; })
            .reduce(function (acc, subSchema) {
            return __spread(acc, _this.getPropertiesFor(subSchema));
        }, []));
        var actualExcess = __spread(reportedExcess).filter(function (property) { return !allowedProperties.has(property); });
    ...
    ValidationService.prototype.getPropertiesFor = function (modelDefinition) {
        var properties = !!modelDefinition && modelDefinition.dataType === 'refObject' ? modelDefinition.properties : {};
    ...
    ValidationService.prototype.validateModel = function (input) {
        var _this = this;
        var name = input.name, value = input.value, refName = input.refName, fieldErrors = input.fieldErrors, _a = input.parent, parent = _a === void 0 ? '' : _a, swaggerConfig = input.minimalSwaggerConfig;
